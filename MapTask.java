...
public class MapTask extends Task {
  ...
  public static final String BONC_MAPRED_MAP_THREADPOOL = "bonc.mapreduce.map.threadpool";
   ...
   * @param <V>
   */
  class TrackedMutlithreadRecordReader<K, V> 
      implements RecordReader<K,V> {
    private RecordReader<K,V> rawIn;
    private Counters.Counter fileInputByteCounter;
    private Counters.Counter inputRecordCounter;
    ...
    TrackedMutlithreadRecordReader(TaskReporter reporter, JobConf job,int threadnum) 
      throws IOException{
      inputRecordCounter = reporter.getCounter(TaskCounter.MAP_INPUT_RECORDS);
      fileInputByteCounter = reporter.getCounter(FileInputFormatCounter.BYTES_READ);
      this.reporter = reporter;
     ...
     // reporter.getInputSplit()
	  CombineFileSplit cfSplit = (CombineFileSplit)reporter.getInputSplit();
	  Path[] paths = { cfSplit.getPath(threadnum)};  
	  long[] starts = { cfSplit.getStartOffsets()[threadnum]};  
	  long[] lengths = { cfSplit.getLength(threadnum)};
	  String[] Locations = {""};
	  if(null != cfSplit.getLocations()){
		String[] Locationstmp = {cfSplit.getLocations()[threadnum]};	
		Locations = Locationstmp;
	  }
	  LOG.info("TrackedMutlithreadRecordReader: " + threadnum+" : "+paths[0]+" "+starts[0]+" "+lengths[0]+" "+Locations);
	 CombineFileSplit newCFSplit = new CombineFileSplit(paths,starts,lengths,Locations);
	 Class[] paramTypes = { CombineFileSplit.class }; // 方法传入的参数
	 Object[] params = {newCFSplit};
	 Constructor con = null ;
	 try {
		 Class<?> aa = reporter.getInputSplit().getClass();
		 con = reporter.getInputSplit().getClass().getConstructor(paramTypes);
		 rawIn = job.getInputFormat().getRecordReader((InputSplit)con.newInstance(params),job, reporter);
		} catch (InvocationTargetException | InstantiationException | IllegalAccessException | IllegalArgumentException|NoSuchMethodException | SecurityException | UnsupportedOperationException e) {
  ...
  public void run(final JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    LOG.info("Multithread useNewApi: " + useNewApi);
    if (useNewApi) {
    	if( !runNewMultithreadMapper(job, splitMetaInfo, umbilical, reporter)){
    		runNewMapper(job, splitMetaInfo, umbilical, reporter);
    	}
    } else {
    	if( !runOldMultithreadMapper(job, splitMetaInfo, umbilical, reporter)){
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  @SuppressWarnings("unchecked")
  private <INKEY,INVALUE,OUTKEY,OUTVALUE>
  boolean runOldMultithreadMapper(final JobConf job,
                    final TaskSplitIndex splitIndex,
                    final TaskUmbilicalProtocol umbilical,
                    TaskReporter reporter
                    ) throws IOException, InterruptedException,
                             ClassNotFoundException {
	  int ThreadsPoolNums  = job.getInt(BONC_MAPRED_MAP_THREADPOOL, 0);  
	  LOG.info("Old Multithread ThreadsPoolNums: " + ThreadsPoolNums);
	  if(ThreadsPoolNums < 1){
		  return false;
	  }
	  int splitnum = 0;  
    ...
    InputSplit inputSplit = getSplitDetails(new Path(splitIndex.getSplitLocation()),
           splitIndex.getStartOffset());
    LOG.info("Old Multithread inputSplit: " + inputSplit.getClass().getName());
    updateJobWithSplit(job, inputSplit);
    reporter.setInputSplit(inputSplit);
   // org.apache.hadoop.mapreduce.lib.input.CombineFileSplit 
    if(CombineFileSplit.class.isAssignableFrom(inputSplit.getClass())){
		LOG.info("Old Multithread split Split is CombineFileSplit");
		//return false;
	}else{
		LOG.info("Old Multithread split Split is not CombineFileSplit");
		return false;
	}
    
    CombineFileSplit cfSplit = (CombineFileSplit)inputSplit;
	splitnum = cfSplit.getPaths().length;
	LOG.info("Old Multithread splitnum: " + splitnum);
	/*if(splitnum < 2){
		return false;
	}*/
	int numberOfThreads = ThreadsPoolNums;	   
	  RecordReader<INKEY,INVALUE> in = isSkipping() ? 
		        new SkippingRecordReader<INKEY,INVALUE>(umbilical, reporter, job) :
		          new TrackedRecordReader<INKEY,INVALUE>(reporter, job);
    job.setBoolean(JobContext.SKIP_RECORDS, isSkipping());
    LOG.info("isSkipping: " + isSkipping());
    ...
    ExecutorService threadPool = Executors.newFixedThreadPool(ThreadsPoolNums);
    OldMultithreadOutputCollector OOC = new OldMultithreadOutputCollector(collector, conf);
	for(int i=0; i < numberOfThreads; i++) {
		//	LOG.info("Multithread splitnum " + i+" : "+paths[0]+" "+starts[0]+" "+lengths[0]+" "+Locations);
		   in = new TrackedMutlithreadRecordReader<INKEY,INVALUE>(reporter, job,i);
		   RunOldMap thread = new <INKEY,INVALUE,OUTKEY,OUTVALUE>RunOldMap(job,in,OOC,reporter,i);
   ...
   */
  private static class OldMultithreadOutputCollector<K,V> implements OutputCollector<K,V> {
    private final Partitioner<K,V> partitioner;
    private final MapOutputCollector<K,V> collector;
    private final int numPartitions;

    @SuppressWarnings("unchecked")
    OldMultithreadOutputCollector(MapOutputCollector<K,V> collector, JobConf conf) {
  ...
  private class NewMultithreadDirectOutputCollector<K,V>
  extends org.apache.hadoop.mapreduce.RecordWriter<K,V> {
    ...
    @SuppressWarnings("unchecked")
    NewMultithreadDirectOutputCollector(MRJobConfig jobContext,
  ...
  private class NewMultithreadOutputCollector<K,V>
    extends org.apache.hadoop.mapreduce.RecordWriter<K,V> {
    private final MapOutputCollector<K,V> collector;
    private final org.apache.hadoop.mapreduce.Partitioner<K,V> partitioner;
    private final int partitions;
    ...
    @SuppressWarnings("unchecked")
    NewMultithreadOutputCollector(org.apache.hadoop.mapreduce.JobContext jobContext,
                       JobConf job,
                       TaskUmbilicalProtocol umbilical,
                       TaskReporter reporter
                       ) throws IOException, ClassNotFoundException {
  ...
  void runNewMapper(final JobConf job,
    ...
    if (job.getNumReduceTasks() == 0) {
      output = 
        new NewMultithreadDirectOutputCollector(taskContext, job, umbilical, reporter);
    } else {
  ...
  @SuppressWarnings("unchecked")
  private <INKEY,INVALUE,OUTKEY,OUTVALUE>
  boolean runNewMultithreadMapper(final JobConf job,
                    final TaskSplitIndex splitIndex,
                    final TaskUmbilicalProtocol umbilical,
                    TaskReporter reporter
                    ) throws IOException, ClassNotFoundException,
                             InterruptedException {
	  int ThreadsPoolNums  = job.getInt(BONC_MAPRED_MAP_THREADPOOL, 0);  
	  LOG.info("Multithread ThreadsPoolNums: " + ThreadsPoolNums);
	  if(ThreadsPoolNums < 1){
		  return false;
	  }
	  int splitnum = 0;
	  ...
	  List<org.apache.hadoop.mapreduce.InputSplit> inputSplits = inputFormat.getSplits(taskContext);
	  
	  if(null == inputSplits){
		  LOG.info("Multithread split inputSplits is null");
		  return false;		 
	  }
		  LOG.info("Multithread split 1 : " +  inputSplits.get(0).getClass().getName());
	...
	// rebuild the input split
	  ...
	  split = getSplitDetails(new Path(splitIndex.getSplitLocation()),
				        splitIndex.getStartOffset());
		LOG.info("Multithread split 2 : " + split.getClass().getName());
		//CombineFileSplit
		if(CombineFileSplit.class.isAssignableFrom(split.getClass())){
			LOG.info("Multithread split Split is CombineFileSplit");
			//return false;
		}else{
			LOG.info("Multithread split Split is not CombineFileSplit");
			return false;
		}
		...
		splitnum = cfSplit.getPaths().length;
		LOG.info("Multithread splitnum: " + splitnum);
		if(splitnum < 2){
			return false;
		}
		
		int numberOfThreads = splitnum;	    
		List<RunMap> runners =  new ArrayList<RunMap>(numberOfThreads);	
		org.apache.hadoop.mapreduce.RecordWriter output = null;
		if (job.getNumReduceTasks() == 0) {
			output = new NewMultithreadDirectOutputCollector(taskContext, job, umbilical, reporter);
		} else {
			output = new NewMultithreadOutputCollector(taskContext, job, umbilical, reporter);
		}
		ExecutorService threadPool = Executors.newFixedThreadPool(ThreadsPoolNums);
	//	LOG.info("Multithread splitnum " + i+" : "+cfSplit.+" "+starts+" "+lengths+" "+Locations);
		
		for(int i=0; i < numberOfThreads; i++) {
			Path[] paths = { cfSplit.getPath(i)};  
			long[] starts = { cfSplit.getStartOffsets()[i]};  
			long[] lengths = { cfSplit.getLength(i)};
			String[] Locations = {""};
			if(null != cfSplit.getLocations()){
				String[] Locationstmp = {cfSplit.getLocations()[i]};	
				Locations = Locationstmp;
			}
			LOG.info("Multithread splitnum " + i+" : "+paths[0]+" "+starts[0]+" "+lengths[0]+" "+Locations);
			CombineFileSplit newCFSplit = new CombineFileSplit(paths,starts,lengths,Locations);
		    RunMap thread = new <INKEY,INVALUE,OUTKEY,OUTVALUE>RunMap(job,taskContext,newCFSplit,output,reporter,i);
		    threadPool.execute(thread);
		 }
		threadPool.shutdown();
		while(!threadPool.isTerminated()){
    ...
    try{
      ...
      setPhase(TaskStatus.Phase.SORT);
      statusUpdate(umbilical);
   //   output.close(taskContext);
   //   output = null;
    } finally {
     // closeQuietly(output, taskContext);
    	output.close(taskContext);
        output = null;
    }
    return true;
  ...
  class  RunOldMap extends Thread {
	  private  MapRunnable runner;//<INKEY,INVALUE,OUTKEY,OUTVALUE> 
	  private int num=0;
	  private  OutputCollector output;
	  private  RecordReader in;//<INKEY,INVALUE>
	  private  TaskReporter reporter;
	  public <INKEY,INVALUE,OUTKEY,OUTVALUE> RunOldMap(final JobConf job,
		  ...
		  this.num=num;
		  LOG.info("Old Multithread init " + num+" ");
	  ...
	  @Override
	    public void run() {
		  try { //runner.run(in, new OldOutputCollector(collector, conf), reporter);
			  LOG.info("Old Multithread run " + num+" ");
			  runner.run(in, output, reporter);
			  in.close();
		      in = null;
		} catch (IOException  e) {
			// TODO Auto-generated catch block
 ...
 class  RunMap extends Thread {
	  private org.apache.hadoop.mapreduce.Mapper mapper ;
	  private org.apache.hadoop.mapreduce.InputFormat inputFormat;
	  private org.apache.hadoop.mapreduce.RecordReader input;
	  private org.apache.hadoop.mapreduce.TaskAttemptContext taskContext;
	  private org.apache.hadoop.mapreduce.InputSplit split;
	  private org.apache.hadoop.mapreduce.RecordWriter output;
	  private  org.apache.hadoop.mapreduce.Mapper.Context  mapperContext;
	  private int num=0;
	  public <INKEY,INVALUE,OUTKEY,OUTVALUE> RunMap(final JobConf job,
              final org.apache.hadoop.mapreduce.TaskAttemptContext taskContext,
              final org.apache.hadoop.mapreduce.InputSplit split,
              final org.apache.hadoop.mapreduce.RecordWriter output,
              TaskReporter reporter,int num
               ) throws ClassNotFoundException, InterruptedException, IOException{
		  this.num=num;
		  LOG.info("Multithread init " + num);
		   mapper = (org.apache.hadoop.mapreduce.Mapper<INKEY,INVALUE,OUTKEY,OUTVALUE>)
				       ReflectionUtils.newInstance(taskContext.getMapperClass(), job);
				   // make the input format
		   inputFormat = (org.apache.hadoop.mapreduce.InputFormat<INKEY,INVALUE>)
				       ReflectionUtils.newInstance(taskContext.getInputFormatClass(), job);
